...
const content = {
  zh: {
    ...
    items: [
      ...
      ["仅第一方聚合反馈", "本站不接入第三方分析、广告像素或行为画像工具；除数据表所列、默认关闭的「另一个视角」实验外，也不会把测评或札记内容发送给 AI 服务。站内另有匿名聚合计数：页面访问、测评开始与完成、结果是否读完，首次完成与 28 天内再次完成这类留存信号，以及访问历史、回看 28 天内完成的旧结果、收藏测评、创建札记草稿这类延续动作，只按日期、界面语言、设备类与测评编号累加；同一设备同一天同一事件最多计一次（登录用户创建札记草稿的计数在服务端的创建动作内完成，同一账号每天最多一次），不记录账号、答案、attemptId、IP 或结果正文，也不用 cookie 关联访问。结果页可匿名提交“图像是否帮助理解”或“AI 视角是否有帮助”的反馈，同样只按维度聚合。"],
    ...
    dataRows: [
      ...
      ["匿名聚合计数：页面访问、测评开始/完成、结果读完、留存与延续信号", "按日累加在服务器上，不含账号、设备或内容标识，无法对应到个人。", "无需删除；服务器仅保留计数本身。"],
      ["「另一个视角」实验（默认关闭）：参与试点测评的维度百分比与命中结果类型", "开启时发送给运营者配置的模型服务商，仅用于当次组装一段参考文字；不发送原始答案、结果正文或任何账号、设备标识，服务器端不保存。", "不使用该功能即可；关闭实验后不再有任何发送。"],
  ...
  en: {
    ...
    items: [
      ...
      ["First-party aggregate feedback only", "The site has no third-party analytics, advertising pixels, or behavioral profiling; apart from the opt-in AI-perspective experiment listed in the data table (off by default), it does not send assessment or journal content to AI services. It also keeps anonymous aggregate counts: page views, assessment starts and completions, whether a result was read to the end, retention signals such as a first completion or a repeat completion within 28 days, and continuation actions such as visiting history, re-reading a result completed within the past 28 days, bookmarking an assessment, or creating a journal draft are summed only by day, interface language, device class, and assessment id. The same event on the same device counts at most once per day (the journal-draft count for signed-in users is recorded inside the server-side draft-creation action, at most once per account per day), with no account ids, answers, attempt ids, IP addresses, or result text, and no cookies for cross-visit linkage. Optional result-page feedback (image helpfulness, AI-perspective helpfulness) is aggregated in the same dimension-only way."],
    ...
    dataRows: [
      ...
      ["Anonymous aggregate counts: page views, assessment starts/completions, results read through, retention and continuation signals", "Accumulated by day on the server with no account, device, or content identifiers; they cannot be linked to a person.", "No deletion needed; only the counters themselves are kept."],
      ["AI-perspective experiment (off by default): dimension percentages and the reached result type of pilot assessments", "When enabled, sent to the operator-configured model provider solely to assemble one reference passage; no raw answers, result text, account, or device identifiers are sent, and the server does not store them.", "Simply do not use the feature; with the experiment disabled nothing is sent."],
